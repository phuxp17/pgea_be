...
module.exports = {
  ...
  fn: async function (inputs, exits) {
    ...
    try {
      ...
      let defaultUserType =
        Conf.getDataFromKey("DEFAULT_USERTYPE_ON_REGISTING") || 3;
      let roleId = [];
      ...
      let link = (
        (sails.config.custom && sails.config.custom.ACTIVE_ACCOUNT_URL) ||
        Conf.getDataFromKey("ACTIVE_ACCOUNT_URL")
      )
        .replace("{{token}}", token)
        .replace("{{account}}", auth.key);
      sails.log.info(
        `[Register] Generated activation link for ${auth.key}: ${link}`,
      );
      sails.log.info(`[Register] Token for ${auth.key}: ${token}`);
