...
module.exports = {
  ...
  fn: async function (inputs, exits) {
    ...
    try {
      ...
      let link = (
        sails.config.custom.RESET_FORGET_ACCOUNT_URL ||
        Conf.getDataFromKey("RESET_FORGET_ACCOUNT_URL")
      )
        .replace("{{token}}", token)
      ...
      pushNotification
        .sendMail(
          auth.user.email,
          sails.__("[Announcement] - Retrieve password via email!"),
          content,
          req,
          {},
        )
      ...
      return res.ok({
        message: sails.__(
          "Please Log In To Your Email And Recover Your Password Via The Link!",
        ),
      });
    } catch (error) {
